...
-- Inserções na tabela ElencoFilme
...
-- Inserções na tabela FilmesGenero
...
-- Criação das constraints de integridade referencial
...
ADD CONSTRAINT FK_FilmesGenero_IdFilme FOREIGN KEY (IdFilme) REFERENCES Filmes(Id),
ADD CONSTRAINT FK_FilmesGenero_IdGenero FOREIGN KEY (IdGenero) REFERENCES Generos(Id);

SELECT * FROM Filmes;
SELECT Nome, Ano FROM Filmes;                                                                      -- 1° Busca 
SELECT Nome, Ano, Duracao FROM Filmes ORDER BY Ano;                                                -- 2° Busca 
SELECT Nome, Ano, Duracao FROM Filmes WHERE Nome = "De Volta para o Futuro";                       -- 3° Busca 
SELECT Nome, Ano, Duracao FROM Filmes WHERE Ano = 1997;                                            -- 4° Busca
SELECT Nome, Ano, Duracao FROM Filmes WHERE Ano > 2000;                                            -- 5° Busca 
SELECT Nome, Ano, Duracao FROM Filmes WHERE Duracao  > 100 AND Duracao < 150 ORDER BY Duracao Asc; -- 6° Busca 

SELECT                                                                                             -- 7° Busca
    ROW_NUMBER() OVER (ORDER BY SUM(Duracao) DESC) AS Id,
    Ano,
    COUNT(*) AS Quantidade
FROM Filmes
GROUP BY Ano
...
SELECT Id, PrimeiroNome, UltimoNome, Genero FROM Atores	 WHERE Genero = 'M';                       -- 8° Busca
SELECT Id, PrimeiroNome, UltimoNome, Genero FROM Atores	 WHERE Genero = 'F' ORDER BY PrimeiroNome; -- 9° Busca

SELECT f.Nome AS Filme, g.Genero                                                                   -- 10° Busca
FROM Filmes f
JOIN FilmesGenero fg ON f.Id = fg.IdFilme
JOIN Generos g ON fg.IdGenero = g.Id;

SELECT f.Nome AS Filme, g.Genero                                                                   -- 11° Busca
FROM Filmes f
JOIN FilmesGenero fg ON f.Id = fg.IdFilme
JOIN Generos g ON fg.IdGenero = g.Id
